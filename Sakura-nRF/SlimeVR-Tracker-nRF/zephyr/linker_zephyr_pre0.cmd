...
MEMORY
    {
    FLASH (rx) : ORIGIN = 0x1000, LENGTH = 0x6d000
    RAM (wx) : ORIGIN = 0x20000000, LENGTH = 0x1f000
    RETAINED_MEM ( rw ) : ORIGIN = (536997888), LENGTH = (4096) DFU_DBL_RESET_MEM ( rw ) : ORIGIN = (536903548), LENGTH = (4)
...
.intList :
{
 ...
 KEEP(*(.gnu.attributes))
    ...
    RETAINED_MEM (NOLOAD) : { __RETAINED_MEM_start = .; KEEP(*(RETAINED_MEM)) KEEP(*(RETAINED_MEM.*)) __RETAINED_MEM_end = .; } > RETAINED_MEM __RETAINED_MEM_size = __RETAINED_MEM_end - __RETAINED_MEM_start; __RETAINED_MEM_load_start = LOADADDR(RETAINED_MEM); DFU_DBL_RESET_MEM (NOLOAD) : { __DFU_DBL_RESET_MEM_start = .; KEEP(*(DFU_DBL_RESET_MEM)) KEEP(*(DFU_DBL_RESET_MEM.*)) __DFU_DBL_RESET_MEM_end = .; } > DFU_DBL_RESET_MEM __DFU_DBL_RESET_MEM_size = __DFU_DBL_RESET_MEM_end - __DFU_DBL_RESET_MEM_start; __DFU_DBL_RESET_MEM_load_start = LOADADDR(DFU_DBL_RESET_MEM);
.last_section :
{
  KEEP(*(.last_section))
} > FLASH
...
noinit (NOLOAD) :
{
    ...
    __kernel_ram_end = 0x20000000 + 0x1f000;
